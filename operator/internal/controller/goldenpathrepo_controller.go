...
// +kubebuilder:rbac:groups=repave.dev,resources=goldenpathrepos,verbs=get;list;watch;create;update;patch;delete
...
func (r *GoldenPathRepoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if repo.Spec.RepoURL == "" && repo.Spec.LocalPath == "" {
		repo.Status.Phase = repavev1alpha1.GoldenPathRepoPhaseError
		repo.Status.Message = "spec.repoURL or spec.localPath is required"
	...
	repo.Status.Message = fmt.Sprintf(
	...
	logger.Info("reconciled GoldenPathRepo", "phase", repo.Status.Phase, "name", req.Name)
	return ctrl.Result{}, nil
}
