...
def test_color_enabled_with_force_color(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "\033[" in gate_status("OK", stream=stream)
    assert "OK" in gate_status("OK", stream=stream)
    assert "\033[" in gate_status("MISSING", stream=stream)
    assert "MISSING" in gate_status("MISSING", stream=stream)
    assert "\033[" in gate_status("MISMATCH", stream=stream)
    assert "MISMATCH" in gate_status("MISMATCH", stream=stream)
