...
def trusted_path(path: Path | str) -> Path:
    ...
    Uses ``os.path.realpath`` plus an explicit ``..`` check. This module is the
    path-injection sanitizer for callers; suppressions mark intentional sinks.
    """
    ...
    resolved = Path(os.path.realpath(candidate, strict=False))  # codeql[py/path-injection]
    if ".." in resolved.parts:
        raise ValueError(
            f"path traversal rejected: {path} resolved to {resolved}; "
            "pass a concrete path without '..'"
        )
    return resolved  # codeql[py/path-injection]
...
def confined_join(root: Path | str, *parts: str | Path) -> Path:
    ...
    base = os.path.realpath(root_resolved, strict=False)  # codeql[py/path-injection]
    for part in parts:
        segment = Path(part)  # codeql[py/path-injection]
    ...
    joined = os.path.join(
        base, *[os.fspath(Path(part)) for part in parts]
    )  # codeql[py/path-injection]
    fullpath = os.path.realpath(joined, strict=False)  # codeql[py/path-injection]
    if os.path.commonpath([base, fullpath]) != base:
        raise ValueError(
            f"path escapes root: {fullpath} is not under {base}; "
            "use a relative path that stays inside the root"
        )
    return Path(fullpath)  # codeql[py/path-injection]
