...
| **Backstage** (ADR 011) | `deploy/k8s/hack/build-backstage-bundle.sh` then `docker build -f backstage/packages/backend/Dockerfile -t ghcr.io/opsdevcode/repave-backstage:TAG backstage/` (CI: `container.yml`) | `repave.backstage.enabled` (**default on**); overlay [`values-backstage.yaml`](values-backstage.yaml) keeps `portal.html: true`, enables catalog fixtures, and same-host `/` + `/api` → engine, `/idp` → Backstage (`app.baseUrl` / `publicBaseUrl`). Do not iframe `/idp`. Kind/smoke overlays keep the flag off. |
...
`assistant.artifacts.enabled` also requires `assistant.draft.enabled` and runs
matched-blueprint gates on candidate files without publishing.
