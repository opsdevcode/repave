# repave roadmap archive

Historical planning detail for **shipped** themes (problem / approach / done-when).
Live status, open work, parking lot, and major-boundary themes:
[`roadmap.md`](roadmap.md).

Do not treat this file as the backlog — update [`roadmap.md`](roadmap.md) first, then move
completed writeups here.
...
| 3 Continuous evolution | Strong | Living roadmap + v1.89 evidence loop ties themes to adoption |
...
**Status:** Shipped on `main` (v1.89 — `/platform/roadmap`, `GET /api/v2/platform/roadmap-evidence`,
theme rows with requesting-team attribution, sunset candidates from funnel conversion).
...
| Anomaly / chargeback | Partial | v1.94 export API + WoW/MoM webhook hooks |
...
**Status:** Shipped on `main` (v1.92 showback — cost snapshot SQL mirror, entity budgets,
`/platform/finops` rollup, service-detail trend vs budget, fleet Prometheus gauges).
...
**Status:** Shipped on `main` (v1.94 chargeback export — `GET /api/v2/platform/finops/export`
FOCUS-friendly CSV/JSON; WoW/MoM snapshot anomalies → audit + `finops_anomaly` webhook).
