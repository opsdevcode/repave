...
import / verify / estate) shipped — Phase 4 is a product call.
...
| **Hosted Backstage IDP** | Named owner | Fleet / import / verify / estate shipped; chart-smoke + GHCR image shipped; flag stays default off ([ADR 011](adr/011-hosted-backstage-idp.md)) |
...
**Status:** Chart-smoke, GHCR image, `/fleet`, `/import`, `/verify`, and
`/estate` shipped ([ADR 011](adr/011-hosted-backstage-idp.md)).
...
chart flag can default on. Fleet, import, verify, and estate already have
pages. Apply stays CLI and operator (`POST /api/v2/upgrades/apply`).
Chart-smoke and GHCR publish already shipped.
